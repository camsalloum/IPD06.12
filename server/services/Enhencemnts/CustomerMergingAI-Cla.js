...
class CustomerMergingAI {
  ...
  async findPotentialDuplicatesWithBlocking(customers, options = {}) {
    logger.info('   🚀 Using performance mode (blocking optimization)');
  ...
  createBlocks(customers, blockSize = 3) {
    ...
    if (this.config.debugMode) {
      logger.info(`   📦 Created ${blocks.size} blocks for ${customers.length} customers`);
    }
  ...
  comparePhoneticTokens(tokens1, tokens2, algorithm) {
    try {
      ...
      if (this.config.debugMode) {
        logger.error('Phonetic comparison error:', error.message);
      }
      return 0;
    }
  }
  ...
  async saveSuggestions(division, suggestions) {
    ...
    try {
      ...
      logger.warn('Batch insert failed, using individual inserts:', error.message);
      ...
      for (const suggestion of suggestions) {
        try {
          ...
          logger.error('Error saving individual suggestion:', individualError.message);
        }
      }
    }
  }
  ...
  async runAccuracyTest(testCases) {
    ...
    logger.info(`   True Positives: ${results.truePositives}`);
    logger.info(`   False Positives: ${results.falsePositives}`);
    logger.info(`   True Negatives: ${results.trueNegatives}`);
    logger.info(`   False Negatives: ${results.falseNegatives}`);
