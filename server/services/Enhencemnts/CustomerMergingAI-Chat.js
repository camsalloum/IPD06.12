...
class CustomerMergingAI {
  ...
  async bulkInsertSuggestions(suggestions) {
    ...
    // });
    logger.info(`bulkInsertSuggestions called with ${suggestions.length} suggestions`);
    // Simulate async op
  ...
  async applyMergeTransaction(mergeInstruction) {
    ...
    logger.info('applyMergeTransaction called', mergeInstruction);
...
if (require.main === module) {
  (async () => {
    ...
    logger.info('Candidates:', JSON.stringify(res, null, 2));
  })();
}
