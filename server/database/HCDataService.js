...
class HCDataService {
  constructor() {
    this.divisionName = 'HC';
    this.pool = null;
  }

  /**
   * Get division pool lazily
   */
  async getPool() {
    if (!this.pool) {
      this.pool = await getDivisionPool(this.divisionName);
    }
    return this.pool;
  }
  ...
  async getProductGroups() {
    try {
      ...
      const result = await pool.query(
        'SELECT DISTINCT INITCAP(LOWER(productgroup)) as productgroup FROM hc_data_excel WHERE productgroup IS NOT NULL AND productgroup != \'\' ORDER BY productgroup'
      );
      return result.rows.map(row => row.productgroup);
    } catch (error) {
      logger.error('Error fetching HC product groups:', error);
      throw error;
    }
  }
  ...
  async getProductGroupsBySalesRep(salesRep) {
    try {
      const pool = await this.getPool();
      const result = await pool.query(
        'SELECT DISTINCT INITCAP(LOWER(productgroup)) as productgroup FROM hc_data_excel WHERE TRIM(UPPER(salesrepname)) = TRIM(UPPER($1)) AND productgroup IS NOT NULL AND productgroup != \'\' ORDER BY productgroup',
        [salesRep]
      );
      return result.rows.map(row => row.productgroup);
    } catch (error) {
      logger.error('Error fetching HC product groups by sales rep:', error);
      throw error;
    }
  }
  ...
  async getSalesData(salesRep, productGroup, valueType, year, month) {
    try {
      const pool = await this.getPool();
      const result = await pool.query(
        'SELECT * FROM hc_data_excel WHERE salesrepname = $1 AND productgroup = $2 AND values_type = $3 AND year = $4 AND month = $5',
        [salesRep, productGroup, valueType, year, month]
      );
      return result.rows;
    } catch (error) {
      logger.error('Error fetching HC sales data:', error);
      throw error;
    }
  }
  ...
  async getSalesDataByValueType(salesRep, productGroup, valueType, year, month, dataType = 'Actual') {
    try {
      ...
      logger.error('Error fetching HC sales data by value type:', error);
      throw error;
    }
  }
  ...
  async getSalesDataForGroup(groupMembers, productGroup, valueType, year, month, dataType = 'Actual') {
    try {
      ...
      logger.error('Error fetching HC sales data for group:', error);
      throw error;
    }
  }
  ...
  async getCustomersBySalesRep(salesRep) {
    try {
      const pool = await this.getPool();
      const result = await pool.query(
        'SELECT DISTINCT customername FROM hc_data_excel WHERE TRIM(UPPER(salesrepname)) = TRIM(UPPER($1)) AND customername IS NOT NULL AND TRIM(customername) != \'\' ORDER BY customername',
        [salesRep]
      );
      return result.rows.map(row => row.customername);
    } catch (error) {
      logger.error('Error fetching HC customers by sales rep:', error);
      throw error;
    }
  }
  ...
  async getCustomersForGroup(groupMembers) {
    try {
      ...
      logger.error('Error fetching HC customers for group:', error);
      throw error;
    }
  }
  ...
  async getCustomerSalesDataByValueType(salesRep, customer, valueType, year, month, dataType = 'Actual') {
    try {
      ...
      logger.error('Error fetching HC customer sales data:', error);
      throw error;
    }
  }
  ...
  async getCustomerSalesDataForGroup(groupMembers, customer, valueType, year, month, dataType = 'Actual') {
    try {
      ...
      logger.error('Error fetching HC customer sales data for group:', error);
      throw error;
    }
  }

  /**
   * Get unique product groups for master data (formatted)
   */
  async getProductGroupsForMasterData() {
    try {
      ...
      logger.error('Error fetching HC product groups for master data:', error);
      throw error;
    }
  }

  /**
   * Get available years for product pricing
   */
  async getProductGroupPricingYears() {
    try {
      ...
      logger.error('Error fetching HC product group pricing years:', error);
      throw error;
    }
  }
  ...
  async getProductGroupPricingAverages(year) {
    ...
    try {
      ...
      logger.error('Error fetching HC product group pricing averages:', error);
      throw error;
    }
  }
  ...
  async getExcludedProductGroups() {
    try {
      const pool = await this.getPool();
      const result = await pool.query(
        'SELECT config_value FROM hc_master_config WHERE config_key = $1',
      ...
      logger.error('Error fetching HC excluded product groups:', error);
      return ['Service Charges', 'Others', 'Other', 'Miscellaneous', 'Service', 'Charges'];
    }
  }
  ...
  async getMaterialPercentages() {
    try {
      const pool = await this.getPool();
      const result = await pool.query(
        'SELECT * FROM hc_material_percentages ORDER BY product_group'
      );
      return result.rows;
    } catch (error) {
      logger.error('Error fetching HC material percentages:', error);
      throw error;
    }
  }
  ...
  async getMaterialPercentage(productGroup) {
    try {
      const pool = await this.getPool();
      const result = await pool.query(
        'SELECT * FROM hc_material_percentages WHERE product_group = $1',
        [productGroup]
      );
      return result.rows[0] || null;
    } catch (error) {
      logger.error('Error fetching HC material percentage:', error);
      throw error;
    }
  }
  ...
  async saveMaterialPercentage(productGroup, percentages, material = '', process = '') {
    try {
      ...
      const query = `
        INSERT INTO hc_material_percentages 
      ...
      logger.error('Error saving HC material percentage:', error);
      throw error;
    }
  }
  ...
  async initializeMaterialPercentages() {
    try {
      ...
      logger.error('Error initializing HC material percentages:', error);
      throw error;
    }
  }
  ...
  async deleteMaterialPercentage(productGroup) {
    try {
      const pool = await this.getPool();
      const result = await pool.query(
        'DELETE FROM hc_material_percentages WHERE product_group = $1 RETURNING *',
        [productGroup]
      );
      return result.rows[0];
    } catch (error) {
      logger.error('Error deleting HC material percentage:', error);
      throw error;
    }
  }
  ...
  async getMasterConfig(key) {
    try {
      ...
      logger.error('Error fetching HC master config:', error);
      throw error;
    }
  }
  ...
  async setMasterConfig(key, value, description = null) {
    try {
      const pool = await this.getPool();
      const query = `
        INSERT INTO hc_master_config (config_key, config_value, description)
        VALUES ($1, $2, $3)
      ...
      logger.error('Error setting HC master config:', error);
      throw error;
    }
  }
  ...
  async getYearlyBudget(salesRep, year, valuesType, groupMembers = null) {
    try {
      ...
      logger.info(`📊 HC Yearly budget query result for ${salesRep} (${year}, ${valuesType}): ${totalValue}`);
      
      return totalValue;
    } catch (error) {
      logger.error('Error fetching HC yearly budget:', error);
      throw error;
    }
  }
}

module.exports = new HCDataService();
