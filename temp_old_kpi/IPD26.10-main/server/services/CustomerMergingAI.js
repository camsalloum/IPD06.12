...
class CustomerMergingAI {
  ...
  async findPotentialDuplicates(customers, rejectedPairs = new Set(), options = {}) {
    ...
    if (useBlocking) {
      console.log('   🚀 Using blocking optimization for faster scanning...');
      const blocks = this.createBlocks(customers);
      console.log(`   📦 Created ${blocks.size} blocks (avg ${Math.round(customers.length / blocks.size)} customers per block)`);
      ...
      console.log('   ⚠️  Using full O(n²) comparison (blocking disabled)');
  ...
  async getRejectedPairs(division) {
    try {
      ...
      console.warn('   ⚠️  Could not load rejected pairs (table may not exist):', error.message);
      return new Set();
    }
  }
