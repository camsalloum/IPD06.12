...
class CustomerMergingAI {
  ...
  async loadConfiguration(division, overrides = {}) {
    ...
    try {
      ...
      console.log('⚠️ Using default configuration (DB load failed):', error.message);
      this.config = { ...defaultConfig, ...overrides };
    }
  ...
  async scanAndSuggestMerges(division, options = {}) {
    ...
    try {
      ...
      console.log(`   🛡️ Protected ${protectedCustomers.size} customers via business rules`);
      ...
      console.log(`   📈 Cache hit rate: ${this.similarityCache.getStats().hitRate}`);
      console.log(`   🔄 Total comparisons: ${this.stats.totalComparisons}\n`);
