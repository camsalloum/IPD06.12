...
class CustomerMergingAI {
  ...
  async scanAndSuggestMerges(division, options = {}) {
    ...
    try {
      ...
      if (rejectedPairs.size > 0) {
        console.log(`   🚫 Found ${rejectedPairs.size} manually rejected pairs (will be skipped)`);
      }
      // --- END NEW ---
  ...
  async findPotentialDuplicates(customers, existingRuleCustomers, rejectedPairs, options = {}) {
    ...
    // --- PASS 1: BLOCKING ---
    console.log('   🏭 Building phonetic blocks...');
    ...
    console.log(`   Comparing pairs within ${blocks.size} blocks...`);
    ...
    console.log(`   Total comparisons made: ${comparisonCount} (vs. ${totalNaiveComparisons} naive)`);

    // --- PASS 3: GRAPH CLUSTERING ---
    console.log(`   Building clusters from ${edges.length} matching edges...`);
  ...
  async getRejectedPairs(division) {
    ...
    try {
      ...
      console.error('Could not load rejected pairs:', error.message);
      return new Set();
    }
    */
  ...
  async saveSuggestions(division, suggestions) {
    ...
    try {
      ...
      console.error('Error saving suggestions (rolled back):', error.message);
    } finally {
