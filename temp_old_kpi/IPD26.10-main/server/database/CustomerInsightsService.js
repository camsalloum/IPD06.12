...
class CustomerInsightsService {
  ...
  async getCustomerInsights(division, year, months, dataType = 'Actual') {
    try {
      ...
      console.log(`📋 Retrieved ${allMergeRules.length} division-wide merge rules`);
    ...
    try {
      ...
      result.rows.forEach(row => {
        ...
        if (normalized.includes('algo food')) {
          console.log(`🔍 DEBUG Algo Food: Found "${customer}" with value ${value.toFixed(2)}, normalized: "${normalized}"`);
        }
        ...
        if (normalizedMap.has(normalized)) {
          ...
          if (normalized.includes('algo food')) {
            console.log(`🔍 DEBUG Algo Food: Duplicate found, summing. New total: ${existing.value.toFixed(2)}`);
          }
        } else {
      ...
      if (normalizedMap.has(algoFoodNormalized)) {
        ...
        console.log(`🔍 DEBUG Algo Food: Final value after SQL aggregation: ${algoFoodData.value.toFixed(2)} (customer name: "${algoFoodData.customer}")`);
      }
  ...
  async getDivisionMergeRules(division) {
    try {
      ...
      console.error('❌ Error fetching division merge rules:', error);
      throw error;
    }
  }
  ...
  applyDivisionMergeRules(customerData, mergeRules) {
    try {
      if (!mergeRules || mergeRules.length === 0) {
        console.log('ℹ️ No merge rules to apply, returning customers as-is');
      ...
      customerData.forEach(customer => {
        ...
        if (!customerMap.has(normalized)) {
          ...
          console.warn(`⚠️ Duplicate normalized customer found: ${normalized}, summing values`);
          customerMap.get(normalized).value += customer.value;
        }
      });
      
      console.log(`🔧 Applying ${mergeRules.length} division-wide merge rules to ${customerMap.size} unique customers`);
      ...
      mergeRules.forEach((rule, ruleIndex) => {
        ...
        uniqueOriginalCustomers.forEach(normalizedOrig => {
          ...
          if (normalizedOrig.includes('algo food')) {
            console.log(`🔍 DEBUG Algo Food in merge rule "${mergedName}": normalized="${normalizedOrig}", exists=${customerMap.has(normalizedOrig)}, processed=${processed.has(normalizedOrig)}`);
            if (customerMap.has(normalizedOrig)) {
              const data = customerMap.get(normalizedOrig);
              console.log(`🔍 DEBUG Algo Food value: ${data.value.toFixed(2)}`);
            }
          }
          ...
          if (customerMap.has(normalizedOrig) && !processed.has(normalizedOrig)) {
            ...
            // Debug logging for Algo Food
            if (normalizedOrig.includes('algo food')) {
              console.log(`🔍 DEBUG Algo Food: Added to merge "${mergedName}", current total: ${totalMergedValue.toFixed(2)}`);
            }
          } else if (customerMap.has(normalizedOrig) && processed.has(normalizedOrig)) {
            // Debug: Customer exists but already processed
            if (normalizedOrig.includes('algo food')) {
              console.warn(`🔍 DEBUG Algo Food: Already processed by another merge rule! Skipping.`);
            }
          }
        });
        ...
        if (matchingCustomers.length > 0) {
          ...
          console.log(`  ✅ Rule ${ruleIndex + 1}: Merged ${matchingCustomers.length} customers into "${mergedName}*": ${totalMergedValue.toFixed(2)}`);
          console.log(`     Customers: ${customerDetails}`);
          if (originalCustomers.length !== uniqueOriginalCustomers.length) {
            console.warn(`     ⚠️ WARNING: Merge rule had ${originalCustomers.length} entries but ${uniqueOriginalCustomers.length} unique customers (duplicates removed)`);
          }
        } else {
          console.log(`  ⚠️ Rule ${ruleIndex + 1}: No matching customers found for merge rule "${mergedName}"`);
        }
      });
      ...
      if (difference > 0.01) { // Allow for floating point errors
        console.error(`❌ CRITICAL: Value mismatch detected! Before merge: ${totalValueBeforeMerge.toFixed(2)}, After merge: ${totalValueAfterMerge.toFixed(2)}, Difference: ${difference.toFixed(2)}`);
        console.error(`   This indicates double-counting. Check merge rules for duplicates.`);
        
        // Debug: Show merged customers with their values
        const mergedCustomers = sorted.filter(c => c.isMerged);
        if (mergedCustomers.length > 0) {
          console.error(`   Merged customers:`);
          mergedCustomers.forEach(mc => {
            console.error(`     - ${mc.name}: ${mc.value.toFixed(2)} (from: ${mc.originalCustomers.join(', ')})`);
          });
        }
      } else {
        console.log(`✅ Value validation passed: ${totalValueBeforeMerge.toFixed(2)} = ${totalValueAfterMerge.toFixed(2)}`);
      }
      
      console.log(`✅ After division-wide merge: ${sorted.length} customers (${sorted.filter(c => c.isMerged).length} merged, ${sorted.filter(c => !c.isMerged).length} individual)`);
      
      return sorted;
    } catch (error) {
      console.error('❌ Error applying division merge rules:', error);
      throw error;
    }
  }
