...
class UniversalSalesByCountryService {
  ...
  static async getSalesRepDivisionalUltraFast(division, salesReps, columns) {
    try {
      ...
      for (const column of columns) {
        ...
        console.log(`🚀 Querying ${column.columnKey}: year=${year}, months=${monthsArray}, type=${dataType}${isEstimateType ? ' (including ACTUAL + ESTIMATE)' : ''}`);
  ...
  static async getProductGroups(division) {
    try {
      ...
      console.error(`Error fetching all product groups for division ${division}:`, error);
      throw error;
    }
  }
