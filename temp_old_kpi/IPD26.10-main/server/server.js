...
const startServer = async () => {
  ...
  if (dbConnected) {
    ...
    console.log('🔄 Synchronizing division tables...');
    try {
      const syncResult = await syncAllTablesToAllDivisions();
      if (syncResult.synced > 0) {
        console.log(`✅ Division sync: ${syncResult.synced} tables created`);
      } else {
        console.log('✅ All divisions are in sync');
      }
    } catch (syncError) {
      console.log('⚠️  Division sync warning:', syncError.message);
    }
  } else {
  ...
  app.listen(PORT, () => {
    ...
    console.log('   - Excel data: /api/financials-fp.xlsx, /api/sales.xlsx');
    console.log('   - Database test: /api/db/test');
    console.log('   - FP data: /api/fp/* (sales-reps, customers, countries, etc.)');
    console.log('   - HC data: /api/hc/* (same structure as FP)');
    console.log('   - AEBF API: /api/aebf/* (health, actual, budget, summary)');
  });
};
