...
router.post('/scan', async (req, res) => {
  try {
    ...
    console.log(`🤖 Running AI scan for division: ${division}`);
...
router.post('/suggestions/:id/approve', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`✅ Suggestion #${id} approved, created rule #${createdRuleId}`);
    ...
    console.error('Error in approve suggestion endpoint:', outerError);
    res.status(500).json({
      success: false,
...
router.post('/suggestions/:id/edit-approve', async (req, res) => {
  ...
  try {
    ...
    console.log(`✏️ Suggestion #${id} edited and approved, created rule #${createdRuleId}`);
...
router.get('/rules/needs-validation', async (req, res) => {
  try {
    ...
    console.error('Error fetching rules needing validation:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/rules/:id/apply-fix', async (req, res) => {
  try {
    ...
    console.log(`🔧 Rule #${id} fixed: "${suggestion.missing}" → "${suggestion.replacement}"`);
...
router.post('/rules/manual', async (req, res) => {
  try {
    ...
    console.log(`✨ Manual rule created: "${mergedName}" (${originalCustomers.length} customers)`);
...
router.put('/rules/:id', async (req, res) => {
  ...
  try {
    ...
    console.log(`✏️ Rule #${id} updated by ${updatedBy || 'Admin'}`);
...
router.post('/validate', async (req, res) => {
  try {
    ...
    console.log(`🔍 Validating merge rules for ${division}...`);
