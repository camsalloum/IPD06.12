...
// Valid divisions list - only FP and HC are active
...
router.get('/health', async (req, res) => {
  try {
    ...
    console.error('❌ AEBF Health check failed:', error);
    res.status(500).json({
      success: false,
      status: 'unhealthy',
      error: error.message
...
router.get('/actual', async (req, res) => {
  try {
    ...
    const validDivisions = VALID_DIVISIONS;
...
router.post('/upload-actual', upload.single('file'), async (req, res) => {
  try {
    ...
    // Validate parameters
    if (!division || !VALID_DIVISIONS.includes(division.toUpperCase())) {
      return res.status(400).json({
        success: false,
    ...
    if (req.body.selectiveMode === 'true' && req.body.selectedYearMonths) {
      ...
      console.log('📅 Selective mode enabled:', selectedYearMonths);
    }
...
router.get('/budget', async (req, res) => {
  try {
    ...
    console.log('📊 Get budget data request:', { division, year, month, search, page, pageSize });
    ...
    console.log(`✅ Found ${result.rows.length} budget records (page ${currentPage} of ${Math.ceil(totalRecords / limit)})`);
...
router.get('/available-months', async (req, res) => {
  ...
  console.log('📅 Get available months request:', { division, year });
  ...
  try {
    ...
    console.log(`✅ Found ${months.length} Actual months:`, months);
...
router.post('/calculate-estimate', async (req, res) => {
  ...
  console.log('📊 Calculate estimate request:', { division, year, selectedMonths, createdBy });
  ...
  try {
    ...
    console.log('📅 All Actual months:', allActualMonths);
    ...
    console.log('📊 Base period months (for averaging):', basePeriodMonths);
    console.log('🎯 Estimate months:', selectedMonths);
    ...
    console.log(`✅ Base period totals:`, totalsResult.rows);
    ...
    console.log('📊 Monthly averages (Simple method - Total÷Months):', monthlyAverages);
    ...
    console.log('📈 Monthly aggregates calculated:', monthlyAggregates);
...
router.post('/save-estimate', async (req, res) => {
  ...
  console.log('💾 Save estimate request:', { division, year, estimateMonths: Object.keys(estimates), approvedBy });
  ...
  try {
    ...
    console.log('📅 Saving estimates for months:', months);
    ...
    console.log('📊 Base period months:', basePeriodMonths);
    ...
    console.log(`✅ Found ${dimensionResult.rows.length} dimension combinations to replicate`);
    ...
    console.log(`🗑️ Deleted ${deleteResult.rowCount} existing estimate records`);
    ...
    console.log(`📊 Base period totals by values_type:`, totalsByType);
    ...
    for (const month of months) {
      ...
      console.log(`✅ Inserted ${recordsForMonth.length} records for month ${month}`);
    }
    
    await client.query('COMMIT');
    
    console.log(`✅ Total inserted: ${totalInserted} estimate records`);
...
async function ensureSalesRepBudgetColumns(division = 'FP') {
  ...
  try {
    ...
    if (!existingColumns.includes('material')) {
      ...
      `);
      console.log(`✅ Added 'material' column to ${tableName}`);
    }
    ...
    if (!existingColumns.includes('process')) {
      ...
      `);
      console.log(`✅ Added 'process' column to ${tableName}`);
    }
    ...
    if (!existingColumns.includes('uploaded_filename')) {
      ...
      `);
      console.log(`✅ Added 'uploaded_filename' column to ${tableName}`);
    }
    ...
    if (!existingColumns.includes('uploaded_at')) {
      ...
      `);
      console.log(`✅ Added 'uploaded_at' column to ${tableName}`);
    }
    ...
    if (constraintCheck.rows.length === 0) {
      ...
      `);
      console.log(`✅ Created unique constraint ${constraintName} on ${tableName}`);
    }

    salesRepBudgetColumnsVerified.add(divisionCode);
    console.log(`✅ Sales rep budget table columns and constraints verified for ${divisionCode}`);
  } catch (error) {
...
async function ensureHtmlBudgetIndexes(division = 'FP') {
  ...
  try {
    ...
    console.log(`✅ HTML Budget indexes created/verified for ${divisionCode}`);
  } catch (error) {
    console.error(`⚠️ Error creating HTML budget indexes for ${divisionCode} (will retry):`, error.message);
...
router.post('/html-budget-customers-all', async (req, res) => {
  try {
    ...
    result.rows.forEach(row => {
      ...
      // Use merged name if available, otherwise convert to proper case
    ...
    console.log(`📊 Loading budget data for ALL sales reps / ${budgetYear}`);
    ...
    console.log(`✅ Found ${budgetResult.rows.length} budget records (all sales reps)`);
    ...
    console.log(`📊 Total rows in response: ${data.length} (all sales reps combined), pricing: ${Object.keys(pricingMap).length} product groups`);
    ...
    console.error('❌ Error fetching all sales reps budget customers:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/html-budget-customers', async (req, res) => {
  try {
    ...
    // Build mapping from original customer names (normalized) to merged customer names
    const norm = (s) => (s || '').toString().trim().toLowerCase();
    // Helper to convert string to Proper Case for display
    const toProperCase = (str) => {
      if (!str) return '';
      return str.toLowerCase().replace(/(?:^|\s|[-/])\w/g, (match) => match.toUpperCase());
    };
    ...
    const budgetYear = parseInt(actualYear) + 1;
    console.log(`📊 Loading budget data for ${division} / ${salesRep} (isGroup: ${!!isGroup}) / ${budgetYear}`);
    console.log(`📊 Querying budget for sales reps: ${salesRepsToQuery.join(', ')}`);
    ...
    console.log(`✅ Found ${budgetResult.rows.length} budget records`);
    ...
    budgetResult.rows.forEach(row => {
      ...
      console.log(`🔍 Budget row: raw=[${row.customer}] -> display=[${displayCustomerName}]`);
      ...
      // Store formatted names for display consistency
      const customerKey = `${displayCustomerName}|${displayCountry}|${displayProductGroup}`;
      budgetOnlyCustomers.add(customerKey);
    });
    ...
    console.log(`🔍 Checking ${budgetOnlyCustomers.size} budget customers against ${data.length} existing data rows`);
    budgetOnlyCustomers.forEach(customerKey => {
      ...
      console.log(`   Budget customer "${customerKey}" -> ${exists ? 'EXISTS in actual data' : 'NOT FOUND (will add)'}`);
      ...
      if (!exists) {
        ...
        console.log(`➕ Adding budget-only: customerKey=[${customerKey}] -> customer=[${customer}]`);
        ...
        console.log(`➕ Added budget-only customer to table: ${customer} (${country}) - ${productGroup}`);
      }
    });
    ...
    console.log(`📊 Total rows in response: ${data.length} (including budget-only customers)`);
    
    // DEBUG: Log budget totals being sent
    const budgetTotal = Object.values(budgetMap).reduce((sum, val) => sum + val, 0);
    console.log(`💰 Budget data being sent: ${Object.keys(budgetMap).length} entries, total = ${budgetTotal} MT`);
    Object.entries(budgetMap).forEach(([key, val]) => {
    ...
    console.error('❌ Error fetching HTML budget customers:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/save-html-budget', async (req, res) => {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    console.error('❌ Save HTML budget error:', error);
...
router.post('/export-html-budget-form', async (req, res) => {
  try {
    ...
    console.log(`📊 Export: Loaded ${pricingResult.rows.length} pricing entries for ${division} year ${pricingYear}`);
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    tfoot tr.budget-amount-total td {
      ...
      font-weight: 700;
      font-size: 12px;
    }foot tr.budget-amount-total td:first-child {
      padding: 8px;
      position: -webkit-sticky;
      position: sticky;
      left: 0;
...
<body>
  ...
  <div id="productGroupChartContainer" class="product-group-chart-container" style="display: none;">
    ...
    <table class="product-group-table" id="customerSummaryTable" style="margin-bottom:32px;">
      ...
      <tbody id="customerSummaryTableBody">
        <!-- Populated by JavaScript -->
      </tbody>
  ...
  <script>
    ...
    // Debounced versions of heavy functions
    ...
    function updateProductGroupChart() {
      ...
      // Build customer totals from actuals and budget
      const actualCustomerTotals = {};
      const budgetCustomerTotals = {};
      // Actuals - sum from actual-row cells (columns 4-15 are months, last column is total)
      document.querySelectorAll('tr.actual-row').forEach(row => {
        const customer = row.querySelector('td:first-child')?.textContent.trim();
        if (!customer) return;
        let total = 0;
        // Month cells are columns 4-15 (indices 3-14), last column (index 15) is the total
      ...
      // Budgets - sum from budget-row and custom-row inputs using data-customer attribute
      // budget-row: inputs have data-customer attribute
      document.querySelectorAll('tr.budget-row input[data-customer][data-month]').forEach(input => {
      ...
      // custom-row: inputs also have data-customer attribute
      document.querySelectorAll('tr.custom-row input[data-customer][data-month]').forEach(input => {
        const customer = input.getAttribute('data-customer');
        if (!customer) return;
        const value = parseFloat(input.value.replace(/,/g, '')) || 0;
        budgetCustomerTotals[customer] = (budgetCustomerTotals[customer] || 0) + value;
      });
      // Union of all customers, sorted by total (actual + budget) descending
      const allCustomers = Array.from(new Set([...Object.keys(actualCustomerTotals), ...Object.keys(budgetCustomerTotals)]))
        .sort((a, b) => {
          const totalA = (actualCustomerTotals[a] || 0) + (budgetCustomerTotals[a] || 0);
          const totalB = (actualCustomerTotals[b] || 0) + (budgetCustomerTotals[b] || 0);
      ...
      if (totalActualCust === 0 && totalBudgetCust > 0) {
        totalVarianceClassCust = 'pg-variance-new';
        totalVarianceTextCust = 'NEW';
        ...
        totalVarianceClassCust = 'pg-variance-neutral';
        totalVarianceTextCust = '0.0%';
      }
      ...
      const totalVarCellCust = document.getElementById('custTotalVariance');
      totalVarCellCust.innerHTML = '<strong><span class="' + totalVarianceClassCust + '">' + totalVarianceTextCust + '</span></strong>';
      totalVarCellCust.className = '';
      
      // Update or create chart (only if we have product group data)
      if (hasProductGroupData) {
    ...
    function attachRowListeners(row) {
      ...
      const applyCustomerSelection = (customer) => {
        ...
        if (customer && countrySelect) {
          console.log('🔍 Searching for customer:', customer);
          ...
          if (actualRow) {
            console.log('✓ Found matching actual-row');
            ...
            if (countryCell) {
              ...
              const matchingOption = Array.from(countrySelect.options).find(opt => 
                opt.value.toLowerCase() === normalizedCountry
              );
              
              if (matchingOption) {
                countrySelect.value = matchingOption.value;
                console.log('✓ Set country dropdown to:', matchingOption.value);
              } else {
                console.warn('⚠️ Country not found in dropdown options:', countryName);
                countrySelect.value = countryName; // Try exact match anyway
              }
              
              countrySelect.disabled = true;
              inputs.forEach(input => {
                input.dataset.country = matchingOption ? matchingOption.value : countryName;
              });
            } else {
              console.warn('⚠️ Country cell not found in actual-row');
            }
          } else {
            console.warn('⚠️ No matching actual-row found for customer:', customer);
      ...
      // Only attach focus listener for validation (blur/input handled by event delegation)
      inputs.forEach(input => {
    ...
    // Note: Input blur/input events are now handled by event delegation on .table-container
    // No need for per-input listeners anymore
    ...
    // EVENT DELEGATION: Single handler on table container instead of per-input listeners
    // This is more efficient and automatically handles dynamically added rows
    const tableContainer = document.querySelector('.table-container');
    if (tableContainer) {
      // Handle input events (typing) with debouncing
      tableContainer.addEventListener('input', function(e) {
        if (e.target.matches('input[data-month]')) {
          // Remove non-numeric characters (allow digits, comma, period)
      ...
      // Handle blur events (format number when leaving field)
      tableContainer.addEventListener('blur', function(e) {
        if (e.target.matches('input[data-month]')) {
          ...
          // Immediate recalculation on blur for accuracy
          recalculateTotals();
          showRecapSummary();
        }
    ...
    document.getElementById('saveFinalBtn').addEventListener('click', function() {
      ...
      allDivs.forEach(div => {
        ...
        const hasTipText = div.innerHTML && div.innerHTML.includes('💡 <strong>Tip:</strong>');
      ...
      alert('✅ Final budget saved!\\n\\nFilename: ' + filename + '\\n\\nThis file is ready for upload to the system.\\n\\n📤 Next step: Upload this file in the Budget Tab.');
    });
  </script>
</body>
</html>`;
    ...
    console.error('❌ Error exporting HTML budget form:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/import-budget-html', async (req, res) => {
  console.log('📥 Import budget HTML request received');
  try {
    ...
    console.log('📋 Current context:', { currentSalesRep, currentDivision });
    ...
    if (!signatureMatch) {
      ...
      console.warn('⚠️ File missing IPD Budget System signature - may be legacy or modified file');
    } else if (signatureMatch[1] !== 'SALES_REP_BUDGET') {
      console.error('❌ Wrong file type: expected SALES_REP_BUDGET, got', signatureMatch[1]);
      return res.status(400).json({
        success: false,
        error: 'Wrong file type. This appears to be a Divisional Budget file. Please use the Divisional Budget import instead.'
      });
    } else {
      console.log('✅ Valid IPD Budget System signature detected');
    }
    ...
    try {
      ...
      if (scriptTagMatch && scriptTagMatch[1]) {
        ...
        if (metaMatch && dataMatch) {
          try {
            ...
            console.error('❌ Failed to JSON.parse metadata from script tag:', e.message);
          ...
          try {
            ...
            console.error('❌ Failed to JSON.parse budget data from script tag:', e.message);
      ...
      if (!metadata || !budgetData) {
        console.error('❌ Invalid HTML format - missing savedBudgetData script or metadata/savedBudget variables');
      ...
      console.log('✅ Parsed metadata and budget data from HTML export');
      console.log('✅ Metadata:', { division: metadata.division, salesRep: metadata.salesRep, budgetYear: metadata.budgetYear });
      console.log('✅ Budget records count:', Array.isArray(budgetData) ? budgetData.length : 0);
    } catch (e) {
      console.error('❌ Error extracting data from HTML:', e);
    ...
    if (currentSalesRep && metadata.salesRep) {
      ...
      if (currentSalesRepNormalized !== fileSalesRepNormalized) {
        console.log('ℹ️ Sales rep differs from current selection - will import to file\'s sales rep');
    ...
    console.log('✅ Sales rep and division validation passed');
    ...
    if (draftCheck) {
      try {
        ...
        if (draftMeta.isDraft === true) {
          console.warn('⚠️ Draft file rejected');
          return res.status(400).json({
            success: false,
    ...
    if (recordErrors.length > 0) {
      console.warn(`⚠️ Skipping ${recordErrors.length} invalid records out of ${budgetData.length}`);
    }
    
    // Use only valid records
    budgetData = validRecords;
    
    console.log(`✅ Validation passed: ${budgetData.length} valid records`);
    ...
    try {
      ...
      console.log('📋 Material/Process lookup map:', materialProcessMap);
      ...
      console.log(`📊 Fetching pricing data for division: ${divisionCode}, year: ${pricingYear}`);
      ...
      try {
        ...
        console.log(`✅ Found ${pricingResult.rows.length} pricing records`);
      } catch (pricingError) {
        console.error('❌ Error querying pricing table:', pricingError);
      ...
      console.log(`📊 Pricing lookup map (Year ${pricingYear}):`, pricingMap);
      ...
      if (missingPricingProducts.size > 0) {
        ...
        console.warn(`⚠️ Missing pricing data for products: ${missingList}`);
      }
      ...
      if (Object.keys(pricingMap).length === 0) {
        ...
        console.warn(`⚠️ No pricing data available for year ${pricingYear}`);
      }
      ...
      if (existingRecords > 0) {
        console.log(`📦 Creating backup of ${existingRecords} existing records before replacement...`);
        ...
        try {
          ...
          if (archiveError.code === '42P01') { // undefined_table
            ...
            console.log(`✅ Archive table created and backup stored`);
          } else {
            console.warn('⚠️ Backup failed (non-critical):', archiveError.message);
            console.warn('⚠️ Rolling back and continuing without backup...');
      ...
      console.log(`🗑️  Deleted ${deleteResult.rowCount} existing sales rep budget records`);
      ...
      // Helper function to normalize names to Proper Case (consistent with actual data)
      const toProperCase = (str) => {
        if (!str) return '';
      ...
      // Normalize sales rep name from metadata
      const normalizedSalesRep = toProperCase(metadata.salesRep);
      
      // Extract filename from metadata if available
      const uploadedFilename = metadata.savedAt ? 
        `BUDGET_${metadata.division.replace(/[^a-zA-Z0-9]/g, '_')}_${normalizedSalesRep.replace(/[^a-zA-Z0-9]/g, '_')}_${metadata.budgetYear}_${new Date(metadata.savedAt).toISOString().replace(/[:.]/g, '-').slice(0, -5)}.html` : 
        null;
      ...
      for (const record of budgetData) {
        ...
        const productGroupKey = normalizedProductGroup.toLowerCase();
        ...
        // Insert Record 1: KGS (always insert) - using normalized names
    ...
    console.error('❌ Error importing budget HTML:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.get('/html-budget-actual-years', async (req, res) => {
  try {
    ...
    console.error('❌ Error fetching HTML budget actual years:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/divisional-html-budget-data', async (req, res) => {
  try {
    ...
    console.log(`📊 Loaded ${pricingResult.rows.length} pricing records for ${division} year ${pricingYear}`);
...
<body>
  ...
  <div class="summary-container">
    ...
    <div class="summary-cards">
      ...
      <!-- MoRM Card -->
      <div class="summary-card">
        <div class="summary-card-title">📈 MoRM</div>
        <div class="summary-card-values">
  ...
  <div class="table-container">
    <table>
          ...
          <th colspan="14" class="legend-header">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              ...
              <div style="display: flex; gap: 8px;">
                <button class="btn" id="saveDraftBtn" style="background: #1677ff;">💾 Save Draft</button>
                <button class="btn" id="saveFinalBtn" style="background: #52c41a;">✓ Save Final</button>
              </div>
              ` : ''}
            </div>
          </th>
      ...
      <!-- Dirham SVG symbol -->
      <svg id="aedSymbol" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" style="display:none"><text x="0" y="15" font-size="16">د.إ</text></svg>
      <tfoot>
        <tr class="actual-total">
  ...
  <script>
    ...
    // Debounce utility for divisional budget
    function debounce(func, wait) {
      let timeout;
      return function executedFunction(...args) {
        const later = () => {
    ...
    console.error('❌ Error exporting divisional HTML budget form:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/import-divisional-budget-html', async (req, res) => {
  console.log('📥 Import divisional budget HTML request received');
  try {
    ...
    if (!signatureMatch) {
      ...
      console.error('❌ Wrong file type: expected DIVISIONAL_BUDGET, got', signatureMatch[1]);
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      console.log('✅ Parsed metadata and budget data from HTML export');
      console.log('✅ Metadata:', { division: metadata.division, budgetYear: metadata.budgetYear });
      console.log('✅ Budget records count:', Array.isArray(budgetData) ? budgetData.length : 0);
    ...
    console.log(`✅ Validation passed: ${recordsToImport.length} valid records`);
    ...
    try {
      ...
      console.log(`✅ Successfully imported divisional budget:`);
    ...
    console.error('❌ Error importing divisional budget HTML:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.get('/budget-sales-reps', async (req, res) => {
  try {
    ...
    console.error('❌ Error fetching budget sales reps:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/budget-sales-rep-recap', async (req, res) => {
  try {
    ...
    console.error('❌ Error fetching sales rep budget recap:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/budget-product-groups', async (req, res) => {
  try {
    ...
    console.error('❌ Error fetching budget product groups:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/actual-product-groups', async (req, res) => {
  try {
    ...
    console.log(`🔍 Fetching actual product groups: division=${division}, year=${actualYear}, salesRep=${salesRep || '__ALL__'}, table=${tables.dataExcel}`);
    ...
    console.log(`✅ Returning ${productGroups.length} product groups`);
    ...
    console.error('❌ Error fetching actual product groups:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.delete('/delete-divisional-budget/:division/:budgetYear', async (req, res) => {
  console.log('🗑️ DELETE divisional budget request received:', req.params);
  ...
  try {
    ...
    `, [division, year]);
    console.log('🔍 Pre-delete check:', preCheckResult.rows);
    ...
    `);
    console.log(`📊 All data in ${tables.divisionalBudget}:`, allDataResult.rows);
    ...
    if (preCheckResult.rows.length > 0) {
      ...
      try {
        ...
        console.log('✅ Divisional budget archived before deletion');
      } catch (archiveError) {
        console.warn('⚠️ Archive failed (non-critical):', archiveError.message);
    ...
    console.log(`✅ Deleted ${deleteResult.rowCount} records from ${tables.divisionalBudget}`);
    ...
    await client.query('ROLLBACK');
    console.error('❌ Error deleting divisional budget:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/bulk-import', async (req, res) => {
  ...
  try {
    ...
    console.log(`📦 Processing ${files.length} files, batch: ${batchId}, status: ${status}`);
    ...
    for (const file of files) {
      try {
        ...
        console.log(`✅ Imported ${Object.keys(groupedData).length} records for ${salesRep} (from ${budgetData.length} raw entries)`);
    ...
    if (saveToFinal && totalImported > 0) {
      ...
      console.log(`✅ Copied ${totalImported} records to main budget table`);
    }
...
router.delete('/bulk-batch/:batchId', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `DELETE FROM ${tables.budgetBulkImport} WHERE batch_id = $1`,
      [batchId]
    );
...
router.post('/bulk-finalize/:batchId', async (req, res) => {
  ...
  try {
    ...
    // First get division from batch
    const divisionResult = await client.query(
      `SELECT DISTINCT division FROM fp_budget_bulk_import WHERE batch_id = $1 LIMIT 1`,
      [batchId]
    );
    ...
    const division = divisionResult.rows[0].division;
    const tables = getTableNames(division);
    
    // Get batch records
    const batchResult = await client.query(
      `SELECT * FROM ${tables.budgetBulkImport} WHERE batch_id = $1 AND status = 'draft'`,
      [batchId]
    );
    ...
    console.log(`📊 Loading pricing data for year ${pricingYear}...`);
    ...
    console.log(`✅ Loaded ${Object.keys(pricingMap).length} pricing records for year ${pricingYear}`);
    
    // Fetch material/process data
    console.log(`📊 Loading material/process data...`);
    ...
    try {
      ...
      console.log(`✅ Loaded ${Object.keys(materialProcessMap).length} material/process records`);
    } catch (matErr) {
      console.warn(`⚠️ Could not load material/process data: ${matErr.message}`);
    }
    ...
    for (const record of batchResult.rows) {
      ...
      await client.query(
        `DELETE FROM ${tables.salesRepBudget} 
         WHERE UPPER(TRIM(salesrepname)) = UPPER($1) AND UPPER(TRIM(customername)) = UPPER($2) AND UPPER(TRIM(countryname)) = UPPER($3) 
         AND UPPER(TRIM(productgroup)) = UPPER($4) AND budget_year = $5 AND UPPER(type) = 'BUDGET'`,
    ...
    await client.query(
      `UPDATE ${tables.budgetBulkImport} SET status = 'final' WHERE batch_id = $1`,
      [batchId]
    );
    
    await client.query('COMMIT');
    
    console.log(`✅ Finalized batch ${batchId}: KGS=${kgsCount}, Amount=${amountCount}, MoRM=${mormCount} records inserted`);
...
'  <div class="header">' +
'    <div class="header-top">' +
'      <h1>📊 Merged Budget Report - ' + batch.division + '</h1>' +
'      <div class="header-actions">' +
'        <input type="text" class="search-box" id="searchBox" placeholder="🔍 Search..." oninput="filterTable()" style="padding: 6px 12px; border: 1px solid #d9d9d9; border-radius: 4px; width: 250px; font-size: 12px;">' +
...
'    <button class="tab-btn active" data-rep="all" onclick="showSalesRep(\'all\')">📊 All (' + records.length + ')</button>' +
    salesRepTabsHTML +
'  </div>' +
'  <div class="table-container">' +
'    <table id="dataTable">' +
