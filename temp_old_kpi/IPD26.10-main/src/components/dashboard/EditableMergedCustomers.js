...
const EditableMergedCustomers = ({ 
    ...
    const loadExistingRules = async () => {
      ...
      try {
        ...
        if (result.success && result.data.length > 0) {
          ...
          setEditableGroups(groups);
          console.log('✅ Loaded existing merge rules from database:', result.data);
        } else {
          console.log('ℹ️ No existing merge rules found in database');
        }
      } catch (error) {
  ...
  const handleSaveMergeRules = async () => {
    ...
    try {
      ...
      console.log('💾 Saving merge rules:', mergeRules);
      console.log('📊 Current editable groups:', editableGroups);

      const response = await fetch('http://localhost:3001/api/customer-merge-rules/save', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        if (onRefreshData) {
          console.log('🔄 Calling onRefreshData to refresh table...');
          onRefreshData();
        } else {
          console.log('⚠️ onRefreshData function not provided');
        }
      } else {
