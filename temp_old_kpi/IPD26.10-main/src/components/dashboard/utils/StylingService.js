...
export class StylingService {
  ...
  static getColumnHeaderStyle(column) {
    ...
    if (column.month === 'Q1' || column.month === 'Q2' || column.month === 'Q3' || column.month === 'Q4') {
      baseStyle = DEFAULT_COLORS.QUARTER;
    } else if (column.month === 'January') {
      baseStyle = DEFAULT_COLORS.JANUARY;
    } else if (column.month === 'Year') {
      baseStyle = DEFAULT_COLORS.YEAR;
    } else if (column.type === 'Budget') {
      baseStyle = DEFAULT_COLORS.BUDGET;
    } else {
      baseStyle = DEFAULT_COLORS.DEFAULT;
    }
  ...
  static getCellBackgroundColor(column) {
    ...
    console.log('🎨 getCellBackgroundColor called with:', {
      column,
      hasCustomColor: !!(column?.customColor || column?.customColorHex),
      customColor: column?.customColor,
      customColorHex: column?.customColorHex,
      month: column?.month,
      type: column?.type
    });
    ...
    if (!column) {
      console.log('🎨 No column provided, using default color:', DEFAULT_CELL_COLORS.DEFAULT);
    ...
    if (column.customColor || column.customColorHex) {
      const palette = getColumnColorPalette(column);
      if (palette.light) {
        console.log('🎨 Using custom color palette →', palette.light);
        return palette.light;
      }
      console.log('🎨 Custom color palette light tone missing, falling back:', column.customColor);
    }
    
    // Default color assignment based on month/type
    if (column.month === 'Q1' || column.month === 'Q2' || column.month === 'Q3' || column.month === 'Q4') {
      console.log('🎨 Using quarter color:', DEFAULT_CELL_COLORS.QUARTER);
      return DEFAULT_CELL_COLORS.QUARTER;
    } else if (column.month === 'January') {
      console.log('🎨 Using January color:', DEFAULT_CELL_COLORS.JANUARY);
      return DEFAULT_CELL_COLORS.JANUARY;
    } else if (column.month === 'Year' || column.month === 'FY') {
      console.log('🎨 Using Year/FY color:', DEFAULT_CELL_COLORS.YEAR);
      return DEFAULT_CELL_COLORS.YEAR;
    } else if (column.month === 'HY1' || column.month === 'HY2') {
      console.log('🎨 Using half-year color (defaulting to blue):', DEFAULT_CELL_COLORS.YEAR);
      return DEFAULT_CELL_COLORS.YEAR;
    } else if (column.type === 'Budget') {
      console.log('🎨 Using Budget color:', DEFAULT_CELL_COLORS.BUDGET);
      return DEFAULT_CELL_COLORS.BUDGET;
    } else if (column.type === 'Forecast') {
      console.log('🎨 Using Forecast color (defaulting to orange):', DEFAULT_CELL_COLORS.QUARTER);
      return DEFAULT_CELL_COLORS.QUARTER;
    } else if (column.type === 'Estimate') {
      console.log('🎨 Using Estimate color (defaulting to yellow):', DEFAULT_CELL_COLORS.JANUARY);
      return DEFAULT_CELL_COLORS.JANUARY;
    }
    ...
    if (fullMonths.includes(column.month)) {
      console.log('🎨 Using full month color:', DEFAULT_CELL_COLORS.JANUARY);
      return DEFAULT_CELL_COLORS.JANUARY;
    }
    
    // Default to blue
    console.log('🎨 Using default color:', DEFAULT_CELL_COLORS.DEFAULT);
    return DEFAULT_CELL_COLORS.DEFAULT;
  }
