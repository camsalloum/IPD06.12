...
<body>
   ...
   function resolveStackedBarColor(period, index, defaultColors, colorSchemes) {
    ...
    function getResponsiveFlags() {
        ...
        // Variable to store scroll position before opening full-screen
        ...
        setTimeout(function() {
            if (chartType === 'divisional-kpis') {
                ...
                // If not, initialize it now (fallback in case pre-init failed)
                if (!charts[chartType] || (charts[chartType] && charts[chartType].isDisposed())) {
        ...
        // Expose global handlers so inline onclick attributes can call them
