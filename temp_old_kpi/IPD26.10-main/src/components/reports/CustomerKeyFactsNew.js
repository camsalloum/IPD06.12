    ...
    if (result.success && result.data && Array.isArray(result.data) && result.data.length > 0) {
      ...
      customers.forEach((c) => {
        ...
        if (mergedCustomerNamesNormalized.has(customerNormalized)) {
          console.log(`🗑️ Filtering out original customer "${c.name}" (matches merged customer)`);
          return;
        }
        ...
        if (allOriginalCustomersNormalized.has(customerNormalized)) {
          console.log(`🗑️ Filtering out customer "${c.name}" (appears in merge rule's originalCustomers)`);
          return;
        }
  ...
  const findings = useMemo(() => {
    ...
    const executiveSummary = {
      ...
      keyRisks: [
        ...
        ...(retentionAnalysis.churnRate > 0.2 ? ['High lost customers rate'] : []),
      ...
      ].slice(0, 4), // Increased to 4 to show more risks
      opportunities: [
