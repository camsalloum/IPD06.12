...
const applySavedMergeRules = async (salesRep, division, customers, dashboardData, extendedColumns) => {
  try {
    const response = await fetch(`http://localhost:3001/api/division-merge-rules?division=${encodeURIComponent(division)}`);
    const result = await response.json();

    if (result.success && result.data.length > 0) {
      console.log(`✅ Found ${result.data.length} saved merge rules for division ${division} - applying aggregation`);
      ...
      customers.forEach(customer => {
        ...
        if (mergedCustomerNamesNormalized.has(customerNormalized)) {
          console.log(`🗑️ Filtering out original customer "${customer}" (matches merged customer)`);
          return;
        }
        ...
        if (allOriginalCustomersNormalized.has(customerNormalized)) {
          console.log(`🗑️ Filtering out customer "${customer}" (appears in merge rule's originalCustomers)`);
          return;
        }
      ...
      console.log(`✅ Applied merge rules: ${processedCustomers.length} rows (from ${customers.length} original customers)`);
    ...
    console.log(`ℹ️ No saved merge rules found for division ${division}, showing all customers individually`);
  ...
  for (let idx = 0; idx < extendedColumns.length; idx++) {
    ...
    if (col.columnType === 'data') {
      ...
      customerGroup.forEach(customerName => {
        ...
        // Sum values for all months in the period for this customer
        // Backend already combines Actual+Estimate when type includes "Estimate"
        monthsToAggregate.forEach(monthName => {
          const month = getMonthNumber(monthName);
          const periodKey = `${col.year}-${month}-${col.type}`;
          const monthValue = dashboardData[customerName]?.[periodKey] || 0;
          if (typeof monthValue === 'number') {
            aggregatedValue += monthValue;
          }
        });
  ...
  useEffect(() => {
    const fetchSalesRepsWithData = async () => {
      ...
      try {
        ...
        const allRepsResponse = await fetch(`http://localhost:3001/api/sales-reps-universal?division=${encodeURIComponent(selectedDivision)}`);
        ...
        if (result.success && result.data) {
          ...
          console.log('🔍 DEBUG: Checking sales data for each rep:');
          Object.keys(result.data).forEach(salesRep => {
            ...
            console.log(`  ${salesRep}: hasSales=${hasSales}, totalSales=${totalSales}, columns=${Object.keys(repData).length}`);
          ...
          console.log(`✅ Loaded ${repsWithData.length} sales reps with actual sales for ${selectedDivision}:`, repsWithData);
          console.log('📊 Sales reps with data (UPPERCASE):', Array.from(salesRepsWithSales));
        } else {
          setSalesRepsWithData([]);
        }
      } catch (error) {
          ...
          return (
            <Tab key={rep} label={toProperCase(rep)}>
              <div className="sales-rep-content">
                {/* Sub-tabs for Tables and Report */}
                <TabsComponent defaultActiveTab={1} className="sub-tabs">
...
const SalesRepTabContent = ({ rep }) => {
  ...
  const aggregateCustomerData = (allCustomerData) => {
    ...
    allCustomerData.forEach(repCustomers => {
      repCustomers.forEach(customer => {
        const customerKey = customer.originalName || customer.name;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Check if rep is a group
        const isRepGroup = isGroup(rep);
        const repsToFetch = isRepGroup ? getGroupMembers(rep) : [rep];

        console.log(`📊 Fetching data for ${isRepGroup ? 'GROUP' : 'INDIVIDUAL'}: ${rep}`, {
          isGroup: isRepGroup,
          members: repsToFetch
        });
        ...
        // If it's a group, fetch data for all members and aggregate
        if (isRepGroup && repsToFetch.length > 1) {
          console.log(`🔄 Fetching data for ${repsToFetch.length} group members:`, repsToFetch);

          // Fetch data for all group members in parallel
          const allMembersDataPromises = repsToFetch.map(async (memberRep) => {
            ...
            if (!response.ok) {
              console.warn(`Failed to fetch data for ${memberRep}:`, response.status);
              return null;
            }
          ...
          const allMembersData = (await Promise.all(allMembersDataPromises)).filter(d => d !== null);

          if (allMembersData.length === 0) {
            throw new Error('No data available for any group member');
          }

          console.log(`✅ Fetched data for ${allMembersData.length} group members, aggregating...`);

          // Extract and aggregate product groups data
          const extendedColumns = buildExtendedColumns(columnOrder);
          ...
          const allAmountData = [];
          const allMormData = [];
          const allCustomerDataPromises = [];

          for (const memberData of allMembersData) {
            const { productGroups, customers, dashboardData, customerData } = memberData;

            // Process product groups for this member
            ...
            allKgsData.push(processProductGroupsForValueType('KGS'));
            allAmountData.push(processProductGroupsForValueType('Amount'));
            allMormData.push(processProductGroupsForValueType('MoRM'));

            // Process customer data for this member
            const dashboardDataForMerge = {};
            customers.forEach(customerName => {
              dashboardDataForMerge[customerName] = customerData[customerName];
            });

            // Apply merge rules and collect the promise
            const customerPromise = applySavedMergeRules(
              memberData.salesRep || repsToFetch[0], // Use member's rep name
              selectedDivision,
              customers,
              dashboardDataForMerge,
              extendedColumns
            ).then(({ customers: finalCustomerData }) => finalCustomerData);

            allCustomerDataPromises.push(customerPromise);
          }

          // Wait for all customer data processing to complete
          const allCustomerData = await Promise.all(allCustomerDataPromises);

          // Aggregate all data
          const aggregatedKgsData = aggregateProductGroupData(allKgsData);
          const aggregatedAmountData = aggregateProductGroupData(allAmountData);
          const aggregatedMormData = aggregateProductGroupData(allMormData);
          const aggregatedCustomerData = aggregateCustomerData(allCustomerData);

          // Sort customers by base period volume
          if (basePeriodIndex != null && basePeriodIndex >= 0) {
            aggregatedCustomerData.sort((a, b) => {
          ...
          setKgsData(aggregatedKgsData);
          setAmountData(aggregatedAmountData);
          setMormData(aggregatedMormData);
          setCustomerData(aggregatedCustomerData);

          console.log(`✅ Aggregated data for group "${rep}" with ${repsToFetch.length} members`);

        } else {
          // Individual sales rep - use existing logic
          const response = await fetch('http://localhost:3001/api/sales-rep-complete-data', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
          ...
          if (result.success) {
            ...
            // Apply saved merge rules to customer data BEFORE processing
            ...
            const { customers: finalCustomerData } = await applySavedMergeRules(
              repsToFetch[0], // Use actual rep name for merge rules
              selectedDivision,
              customers, // Raw customer names
            ...
            console.log(`✅ Loaded complete data for ${repsToFetch[0]} using optimized batch API`);
          } else {
