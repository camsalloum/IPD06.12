...
const SalesByCustomerTableNew = ({ hideHeader = false }) => {
  ...
  const fetchMergeRules = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/division-merge-rules/rules?division=${encodeURIComponent(selectedDivision)}`);
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        console.log(`✅ Loaded ${mappedRules.length} division-wide merge rules for ${selectedDivision}`);
        return mappedRules; // Return rules for immediate use
      } else {
        setMergeRules([]);
        setMergeRulesLoaded(true);
        console.log(`ℹ️ No merge rules found for division ${selectedDivision}`);
        return [];
      }
    } catch (err) {
  ...
  const applyMergeRulesToCustomersWithRules = (rawCustomers, rulesToApply) => {
    if (!rulesToApply || rulesToApply.length === 0) {
      console.log('ℹ️ No merge rules, returning raw customers:', rawCustomers.length);
      return rawCustomers;
    }
    ...
    console.log(`🔍 Applying ${rulesToApply.length} merge rules to ${rawCustomers.length} raw customers`);
    ...
    rulesToApply.forEach(rule => {
      ...
      if (existingCustomers.length >= 1) {
        ...
        console.log(`✅ Merged: "${mergedDisplayName}" <- [${existingCustomers.join(', ')}]`);
      }
    });
    ...
    console.log(`📊 After merge rules: ${processedCustomers.length} customers (${processedCustomers.filter(c => c.endsWith('*')).length} merged, ${processedCustomers.filter(c => !c.endsWith('*')).length} individual)`);
    ...
    const deduped = processedCustomers.filter(customer => {
      ...
      if (shouldRemove) {
        console.log(`🗑️ Removing duplicate: "${customer}" (matches merged customer)`);
      }
      return !shouldRemove;
    });
    ...
    console.log(`✅ Final customer list: ${final.length} customers (removed ${processedCustomers.length - final.length} duplicates)`);
    return final;
  };
  ...
  const extractCustomersFromSalesData = (salesDataObject, mergeRulesToApply = null) => {
    ...
    console.log(`✅ Extracted ${mergedCustomerNames.length} customers from sales data`);
  ...
  const fetchSalesRepMapping = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/customer-sales-rep-mapping?division=${encodeURIComponent(selectedDivision)}`);
      const result = await response.json();
      ...
      if (result.success && result.data) {
        ...
        console.log(`✅ Loaded ${Object.keys(normMap).length} sales rep mappings (${mergedCount} merged assignments)`);
      } else {
