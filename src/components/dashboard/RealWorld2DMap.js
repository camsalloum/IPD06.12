...
const RealWorld2DMap = () => {
  ...
  console.log('🔄 RealWorld2DMap rendered with:', { selectedDivision, countryDataLength: countryData.length, selectedPeriodIndex });
  ...
  const fetchCountries = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`✅ Loaded ${countryNames.length} countries for ${selectedDivision}:`, countryNames);
      } else {
  ...
  const fetchSalesData = useCallback(async (periodColumn) => {
    console.log('🚀 RealWorld2DMap: fetchSalesData called with:', { selectedDivision, periodColumn });
    if (!selectedDivision || !periodColumn) {
      console.log('❌ RealWorld2DMap: Missing requirements:', { selectedDivision, periodColumn });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`✅ RealWorld2DMap: Loaded sales data for ${transformedData.length} countries for ${periodColumn.year}/${periodColumn.month} (${periodColumn.type}):`, transformedData);
      } else {
  ...
  useEffect(() => {
    console.log('📅 Period selection effect:', { columnOrderLength: columnOrder.length, basePeriodIndex });
    if (columnOrder.length > 0 && basePeriodIndex !== null) {
      console.log('🎯 Setting base period index:', basePeriodIndex);
      setSelectedPeriodIndex(basePeriodIndex);
    } else if (columnOrder.length > 0) {
      console.log('🎯 Setting default period index: 0');
  ...
  useEffect(() => {
    console.log('🔄 RealWorld2DMap: Period effect triggered:', { periodsLength: periods.length, selectedPeriodIndex, periods });
    if (periods.length > 0 && selectedPeriodIndex < periods.length) {
      const selectedPeriod = periods[selectedPeriodIndex];
      console.log('🔄 RealWorld2DMap: Fetching data for selected period:', selectedPeriod);
      fetchSalesData(selectedPeriod);
    } else {
      console.log('❌ RealWorld2DMap: Not fetching data - conditions not met');
    }
  }, [selectedPeriodIndex, periods, fetchSalesData]);

  // Memoized sales countries calculation
  const salesCountries = useMemo(() => {
    console.log('🧮 Calculating sales countries from data:', countryData);
    ...
    const result = countryData
      .map(item => {
        ...
        console.log(`📍 ${item.country}: ${item.values} (${percentage.toFixed(2)}%)`);
        return {
          name: item.country,
    ...
    console.log('📊 Filtered countries (>=0.1%):', result);
    ...
    console.log('Country data from database:', countryData.length, 'countries');
        ...
        if (mapContainer._leaflet_id) {
          try {
            ...
            if (existingMap && typeof existingMap.remove === 'function') {
              existingMap.remove();
              console.log('🗺️ Existing map removed successfully');
            }
          } catch (e) {
          ...
          try {
            ...
            setTimeout(() => {
              try {
                ...
                map.invalidateSize();
                console.log('🗺️ Map size invalidated successfully');
              } catch (e) {
            ...
            console.log('🗺️ Map initialization completed successfully');
          } catch (err) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      salesCountries.forEach(({ name, percentage, coords }) => {
        ...
        if (!c) {
          console.log(`❌ RealWorld2DMap: No coordinates found for ${name} (normalized: ${normName})`);
          return;
        }
  ...
  return (
    <div className="real-map-container">
      ...
      {console.log('🎛️ Rendering period buttons:', { periodsLength: periods.length, periods })}
      {periods.length > 0 && (
