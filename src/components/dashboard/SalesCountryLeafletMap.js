...
const SalesCountryLeafletMap = ({ 
  ...
  useEffect(() => {
    ...
    if (!mapContainerRef.current) {
      console.log('🗺️ Leaflet: Container not ready yet');
      return;
    }

    // Check if this container already has a map (Leaflet adds _leaflet_id)
    if (mapContainerRef.current._leaflet_id) {
      console.log('🗺️ Leaflet: Map already exists on this container, reusing...');
    ...
    console.log('🗺️ Leaflet: Initializing map...');
    ...
    try {
      ...
      setTimeout(() => {
        if (map && map._container) {
          map.invalidateSize();
          console.log('🗺️ Leaflet: Map size invalidated');
        }
      }, 200);
    ...
    return () => {
      ...
      setTimeout(() => {
        ...
        if (!isMounted && mapRef.current && !mapContainerRef.current?._leaflet_id) {
          console.log('🗺️ Leaflet: Final cleanup - removing map');
