      ...
      if (rep && selectedDivision) {
        try {
          ...
          if (result.success && result.data && result.data.length > 0) {
            console.log(`✅ Applying ${result.data.length} division-wide merge rules to customer data in PerformanceDashboard`);
            ...
            rawCustomerList.forEach(customer => {
              ...
              if (mergedCustomerNamesNormalized.has(customerNormalized)) {
                console.log(`🗑️ Filtering out original customer "${customer.name}" (matches merged customer)`);
                return;
              }
