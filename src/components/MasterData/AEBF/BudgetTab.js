...
const BudgetTab = () => {
  ...
  const fetchHtmlSalesRepOptions = async (
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/sales-rep-divisional-ultra-fast', {
  ...
  const fetchHtmlFilterOptions = async () => {
    ...
    try {
      const [filterResponse, groupsResponse, salesRepsResponse] = await Promise.all([
        axios.get('http://localhost:3001/api/aebf/filter-options', {
          params: { division: selectedDivision, type: 'Actual' }
        }),
        axios.get('http://localhost:3001/api/sales-rep-groups-universal', {
          params: { division: selectedDivision }
        }),
        axios.get('http://localhost:3001/api/sales-reps-universal', {
          params: { division: selectedDivision }
        })
      ]);
  ...
  const handleFileSelect = (file) => {
    ...
    console.log('Setting upload modal visible to true');
  ...
  const handleTransformLoad = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/aebf/upload-budget', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
  ...
  // Fetch ALL product groups for division (unfiltered, for custom rows)
  const fetchAllProductGroups = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3001/api/product-groups-universal', {
  ...
  useEffect(() => {
    ...
    // Also fetch all product groups for custom rows
    if (selectedDivision) {
  ...
  const fetchAllSalesRepsData = useCallback(async () => {
    ...
    try {
      ...
      const salesRepsResponse = await axios.get('http://localhost:3001/api/sales-reps-universal', {
        params: { division: selectedDivision }
      });
      ...
      const response = await axios.post('http://localhost:3001/api/aebf/html-budget-customers-all', {
        division: selectedDivision,
      ...
      if (response.data.success) {
        console.log('📊 Backend returned', response.data.data?.length, 'rows for all sales reps');
        ...
        console.log('📊 Loaded pricing data from backend:', Object.keys(pricingDataFromBackend).length, 'product groups');
        ...
        console.log('📊 Loaded budget data from backend:', Object.keys(budgetDataFromBackend).length, 'entries');
  ...
  const fetchHtmlTableData = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        console.log('📊 Backend returned', response.data.data?.length, 'rows');
        console.log('📊 Sample rows:', response.data.data?.slice(0, 3));
        setHtmlTableData(response.data.data || []);
        ...
        const budgetDataFromBackend = response.data.budgetData || {};
        console.log('📊 Loaded budget data from backend:', Object.keys(budgetDataFromBackend).length, 'entries');
        console.log('📊 Sample budget keys:', Object.keys(budgetDataFromBackend).slice(0, 3));
        ...
        (response.data.data || []).forEach(row => {
          for (let month = 1; month <= 12; month++) {
            ...
            if (backendValue !== undefined && backendValue !== null) {
              console.log(`  ✅ Loaded budget: ${key} = ${backendValue}`);
            }
          }
        });
  ...
  const handleSaveBudget = async () => {
    ...
    setHtmlSaving(true);
    try {
      const response = await axios.post('http://localhost:3001/api/aebf/save-html-budget', {
        division: selectedDivision,
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          console.log('✅ Filename validated:', file.name);
          ...
          try {
            ...
            console.log('📋 Parsed metadata from file:', {
              division: metadata.division,
              salesRep: metadata.salesRep,
              budgetYear: metadata.budgetYear,
              actualYear: metadata.actualYear
            });
          } catch (e) {
          ...
          console.log('✅ Backend response received:', checkResponse.data);
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/api/budget-draft/save-draft', {
        division: selectedDivision,
        salesRep: htmlFilters.salesRep,
  ...
  const submitFinalBudget = async () => {
    ...
    console.log('🚀 Submit Final Budget clicked');
    console.log('Filters:', { selectedDivision, salesRep: htmlFilters.salesRep, actualYear: htmlFilters.actualYear, isAllSalesReps });
    console.log('Budget Data:', htmlBudgetData);
    console.log('Budget Data Keys:', Object.keys(htmlBudgetData).length);
    ...
    console.log('✅ Submit confirmation modal state set to visible');
  };
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (isAllSalesReps) {
        console.log('📤 All Sales Reps mode - grouping data by sales rep...');
        ...
        console.log(`📊 Found ${salesRepsList.length} sales reps to submit:`, salesRepsList);
        ...
        for (const salesRep of salesRepsList) {
          ...
          console.log(`📤 Submitting budget for ${salesRep}:`, Object.keys(repBudgetData).length, 'entries');
          ...
          try {
            ...
            if (response.data.success) {
              ...
              try {
                await axios.delete(
                  `http://localhost:3001/api/budget-draft/delete-draft/${selectedDivision}/${salesRep}/${parseInt(htmlFilters.actualYear) + 1}`
                );
              } catch (e) {
      ...
      if (response.data.success) {
        ...
        try {
          await axios.delete(
            `http://localhost:3001/api/budget-draft/delete-draft/${selectedDivision}/${htmlFilters.salesRep}/${parseInt(htmlFilters.actualYear) + 1}`
          );
          setHasDraft(false);
          setDraftStatus('saved');
        } catch (deleteError) {
  ...
  const handleExportHtmlForm = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:3001/api/aebf/export-html-budget-form',
        {
          division: selectedDivision,
          actualYear: htmlFilters.actualYear,
  ...
  const fetchDivisionalHtmlActualYears = useCallback(async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3001/api/aebf/html-budget-actual-years', {
        params: { division: selectedDivision }
      });
  ...
  const handleExportDivisionalHtmlForm = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:3001/api/aebf/export-divisional-html-budget-form',
        {
          division: selectedDivision,
          actualYear: divisionalHtmlFilters.actualYear,
  ...
  const handleImportDivisionalFilledHtml = async (file) => {
    ...
    console.log('✅ Divisional budget filename validated:', file.name);
    
    const reader = new FileReader();
    reader.onload = async (e) => {
      try {
        const htmlContent = e.target.result;
        console.log('📄 Divisional budget file read successfully, size:', htmlContent.length, 'characters');
        ...
        const checkResponse = await axios.post('http://localhost:3001/api/aebf/import-divisional-budget-html', {
  ...
  const performDivisionalImport = async (htmlContent) => {
    try {
      const response = await axios.post('http://localhost:3001/api/aebf/import-divisional-budget-html', {
        htmlContent: htmlContent,
        confirmReplace: true
      });
  ...
  const fetchRecapYears = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3001/api/aebf/budget-years', {
        params: { division: selectedDivision }
      });
  ...
  const fetchRecapSalesReps = async (year) => {
    ...
    try {
      const response = await axios.get('http://localhost:3001/api/aebf/budget-sales-reps', {
  ...
  const fetchRecapData = async (year, salesRep) => {
    ...
    if (!salesRep || salesRep === '__ALL__') {
      ...
      try {
        ...
        const salesRepsResponse = await axios.get('http://localhost:3001/api/aebf/budget-sales-reps', {
          params: { division: selectedDivision, budgetYear: year }
        });
        ...
        const recapPromises = allSalesReps.map(rep => 
          axios.post('http://localhost:3001/api/aebf/budget-sales-rep-recap', {
            division: selectedDivision,
            budgetYear: year,
            salesRep: rep
          })
    ...
    try {
      ...
      if (groupMembers && groupMembers.length > 0) {
        ...
        const response = await axios.post('http://localhost:3001/api/aebf/budget-sales-rep-recap', {
          division: selectedDivision,
          budgetYear: year,
  ...
  const fetchRecapProductGroups = async (year, salesRep) => {
    ...
    try {
      ...
      const pgPromises = rawSalesReps.map(rep =>
        axios.post('http://localhost:3001/api/aebf/budget-product-groups', {
          division: selectedDivision,
          budgetYear: year,
          salesRep: rep
        })
      ...
      console.log('Budget product groups from API:', results.map(r => r.data.productGroups));
      console.log('Aggregated pgMap:', pgMap);
      ...
      if (!salesRep || salesRep === '__ALL__') {
        ...
        const actualResponse = await axios.post('http://localhost:3001/api/aebf/actual-product-groups', {
          division: selectedDivision,
        ...
        const actualPromises = rawSalesReps.map(rep =>
          axios.post('http://localhost:3001/api/aebf/actual-product-groups', {
            division: selectedDivision,
            actualYear: actualYear,
            salesRep: rep
          })
  ...
  const handleDeleteBudget = () => {
    ...
    console.log('Filters:', { selectedDivision, salesRep: htmlFilters.salesRep, actualYear: htmlFilters.actualYear, isAllSalesReps, targetSalesRep });
    ...
    const budgetYear = parseInt(htmlFilters.actualYear) + 1;
    console.log('Opening delete confirmation modal for budget year:', budgetYear, 'salesRep:', effectiveSalesRep);
    ...
    (async () => {
      ...
      try {
        const url = `http://localhost:3001/api/budget-draft/delete-final/${selectedDivision}/${effectiveSalesRep}/${budgetYear}`;
  ...
  const handleDeleteDivisionalBudget = () => {
    console.log('🗑️ Delete divisional budget button clicked');
    console.log('Filters:', { selectedDivision, actualYear: divisionalHtmlFilters.actualYear });
    ...
    const budgetYear = parseInt(divisionalHtmlFilters.actualYear) + 1;
    console.log('Opening delete confirmation modal for divisional budget year:', budgetYear);
    ...
    (async () => {
      console.log('🗑️ Delete confirmed, sending request...');
      try {
        const url = `http://localhost:3001/api/aebf/delete-divisional-budget/${encodeURIComponent(selectedDivision)}/${budgetYear}`;
        console.log('DELETE URL:', url);
        ...
        if (response.data.success) {
          ...
          console.log('Refreshing divisional table data...');
          await fetchDivisionalHtmlTableData();
          console.log('Table refreshed');
        } else {
    ...
    setHtmlCustomRows(prev => prev.map(row => {
      if (row.id === rowId) {
        ...
        if (!isNewCustomer && customer) {
          console.log('🔍 Looking for country for customer:', customer);
          ...
          if (existingRow) {
            console.log('✓ Found matching customer, auto-filling country:', existingRow.country);
            updatedRow.country = existingRow.country;
          } else {
            console.warn('⚠️ No matching customer found in table data');
            console.log('   Available customers:', htmlTableData.map(r => r.customer).slice(0, 10));
          }
