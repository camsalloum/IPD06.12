...
router.get('/divisions/impact/:code', authenticate, requireRole('admin'), async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('No main data found for division:', code);
    }
    ...
    try {
      ...
      console.log('No budget data found for division:', code);
    }
    ...
    console.error('Check division impact error:', error);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/divisions', authenticate, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    for (const code of deletedCodes) {
      console.log(`Cascading delete for division: ${code}`);
      ...
      try {
        ...
        console.log(`✅ Deleted database for division: ${code}`);
      } catch (error) {
        console.log(`⚠️ Error deleting database for ${code}:`, error.message);
    ...
    for (const newDiv of addedDivisions) {
      ...
      try {
        ...
        console.log(`✅ Division ${newDiv.code} (${newDiv.name}) database created successfully!`);
      } catch (error) {
        console.error(`❌ Error creating division ${newDiv.code}:`, error.message);
        throw error;
      }
    }
    ...
    console.error('Update divisions error:', error);
    res.status(500).json({ error: error.message });
  } finally {
    client.release();
  }
