...
const fetchPricingMap = async (client, division, divisionCode, pricingYear) => {
  ...
  try {
    ...
    console.warn(`⚠️ Pricing lookup failed for division ${divisionCode}, year ${pricingYear}:`, error.message);
    return {};
  }
};
...
const saveDivisionalBudget = async (client, options) => {
  ...
  console.log(`📝 Preparing divisional budget records for merge/upsert...`);
  ...
  if (rowsToUpsert.length > 0) {
    console.log(`🚀 Executing batch upsert for ${rowsToUpsert.length} records...`);
    
    for (let i = 0; i < rowsToUpsert.length; i += CHUNK_SIZE) {
      const chunk = rowsToUpsert.slice(i, i + CHUNK_SIZE);
      
      // Generate placeholders: ($1, $2, ...), ($10, $11, ...)
      const placeholders = chunk.map((_, rIndex) => 
        `(${Array.from({ length: 10 }, (_, cIndex) => `$${rIndex * 10 + cIndex + 1}`).join(', ')})`
      ).join(', ');
  ...
  console.log(`✅ Divisional budget saved successfully:`);
  console.log(`   - Processed/Upserted: ${insertedCount} records`);
