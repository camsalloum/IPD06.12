...
router.post('/save-draft', async (req, res) => {
  ...
  try {
    ...
    if (!division || !salesRep || !budgetYear) {
      console.error('❌ Missing required fields:', { division, salesRep, budgetYear });
    ...
    console.log('📋 Sample budget data entries:', Object.entries(budgetData || {}).slice(0, 3));
    ...
    console.log('📋 Using table:', tables.salesRepBudgetDraft);
    ...
    try {
      ...
      console.log(`📝 Processing ${entries.length} budget entries...`);
      ...
      for (const [key, value] of entries) {
        ...
        if (key.includes('|')) {
          ...
          if (!row || !row.customer || !row.country || !row.productGroup) {
            console.warn(`⚠️ Custom row not found or incomplete: ${rowId}`);
            skippedCount++;
            continue;
          }
          ...
          // Unknown format
          console.warn(`⚠️ Unknown key format: ${key}`);
          skippedCount++;
          continue;
        }
        
        if (isNaN(month) || month < 1 || month > 12) {
          console.warn(`⚠️ Invalid month: ${month} for key ${key}`);
          skippedCount++;
          continue;
        }
        if (!customer || !country || !productGroup) {
          console.warn(`⚠️ Missing required fields for key ${key}:`, { customer, country, productGroup });
          skippedCount++;
          continue;
        }
        ...
        try {
          ...
          errorCount++;
          console.error(`❌ Error inserting record for key ${key}:`, {
      ...
      console.log(`✅ Draft save complete: inserted=${insertedCount}, skipped=${skippedCount}, errors=${errorCount}`);
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        `);
        console.log(`✅ Found ${materialProcessResult.rows.length} material/process records`);
      } catch (tableError) {
        console.error('❌ Error querying material table:', tableError);
      ...
      for (let i = 0; i < draftResult.rows.length; i++) {
        ...
        try {
          ...
          if (isNaN(kgsValue)) {
            console.warn(`⚠️ Skipping record ${i + 1}: Invalid KGS value (${draftRow.values})`);
            continue;
          }
          
          if (!draftRow.customername || !draftRow.countryname || !draftRow.productgroup) {
            console.warn(`⚠️ Skipping record ${i + 1}: Missing required fields`, draftRow);
            continue;
          }
          ...
          try {
            ...
            console.error(`❌ Error inserting KGS record ${i + 1}:`, insertError);
      ...
      if (insertedKGS === 0) {
        console.error('❌ No records were inserted. All rows may have failed validation.');
        await client.query('ROLLBACK');
        // Don't release here - let finally block handle it
        return res.status(400).json({
          success: false,
    ...
    try {
      ...
      console.error('Error releasing DB client in submit-final finally block:', releaseErr);
    }
  }
});
...
router.delete('/delete-final/:division/:salesRep/:budgetYear', async (req, res) => {
  console.log('🗑️ DELETE budget request received:', req.params);
  try {
    const { division, salesRep, budgetYear } = req.params;
    
    console.log(`Deleting budget for: Division=${division}, SalesRep=${salesRep}, Year=${budgetYear}`);
    ...
    console.log(`✅ Deleted ${finalResult.rowCount} records from ${tables.salesRepBudget}`);
    ...
    console.log(`✅ Deleted ${draftResult.rowCount} records from ${tables.salesRepBudgetDraft}`);
    ...
    console.error('❌ Error deleting final budget:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
