    ...
    try {
      ...
      if (groupsResponse.ok) {
        ...
        if (salesReps.length > 0) {
          ...
          console.log(`✅ Loaded sales rep groups for ${selectedDivision}:`, cleanedGroups);
        } else {
          setSalesRepGroups(loadedGroups);
          console.log(`✅ Loaded sales rep groups for ${selectedDivision}:`, loadedGroups);
        }
      } else {
    ...
    for (const [groupName, members] of Object.entries(groups)) {
      ...
      if (validMembers.length > 0) {
        ...
        if (validMembers.length < members.length) {
          ...
          console.log(`⚠️ Group "${groupName}": Removed ${removedMembers.length} member(s) no longer in DB:`, removedMembers);
        }
      } else {
        // Group has no valid members - mark for deletion
        groupsToDelete.push(groupName);
        console.log(`🗑️ Group "${groupName}" will be deleted: No members found in current database`);
      }
    }
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '30px' }}>
        <button 
          style={{
            padding: '15px 25px',
            backgroundColor: activeTab === 'pricing' ? '#007bff' : 'white',
        ...
        <button
          style={{
            padding: '15px 25px',
            backgroundColor: activeTab === 'customer-merging' ? '#667eea' : 'white',
            color: activeTab === 'customer-merging' ? 'white' : '#667eea',
            border: '2px solid #667eea',
            borderRadius: '8px',
            cursor: 'pointer',
            fontWeight: '600'
          }}
          onClick={() => setActiveTab('customer-merging')}
        >
          🤖 Customer Merging
        </button>
      </div>

      {/* Tab 0: Product Pricing */}
      {activeTab === 'pricing' && (
