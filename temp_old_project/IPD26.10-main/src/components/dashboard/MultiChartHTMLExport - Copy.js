...
const getSalesByCountryTableStyles = async () => {
  ...
  // Fallback: use shared style chunk shipped with the app
  console.warn('⚠️ Using fallback SALES_BY_COUNTRY_CSS_CONTENT');
    ...
    for (const sheet of styleSheets) {
      try {
        ...
        const sbsrStyles = rules
          .filter(rule => {
            const cssText = rule.cssText || '';
            // Match Sales by Sales Rep specific classes and variables
    ...
    for (const sheet of styleSheets) {
      try {
        ...
        const plStyles = rules
          .filter(rule => {
            const cssText = rule.cssText || '';
            // Match P&L Table specific classes and variables
...
const {
  ...
  const generateOutstandingKPISummary = async () => {
    try {
      ...
      // Check overlay first, then regular DOM
      const kpiComponentFinal = document.querySelector('.divisional-dashboard__overlay .kpi-dashboard') || 
                                document.querySelector('.kpi-dashboard');
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      console.log('🔥 Step: Capturing KPI data from DOM...');
      let kpiSummaryHTML = '<div class="placeholder-content"><h3>KPI Summary</h3><p>KPI data not available.</p></div>';
      try {
        // Look for KPI dashboard in the current page (no navigation needed)
        const kpiContainer = document.querySelector('.kpi-dashboard, .divisional-dashboard__overlay .kpi-dashboard');
        if (kpiContainer) {
          const clonedKPI = kpiContainer.cloneNode(true);
          kpiSummaryHTML = clonedKPI.outerHTML;
          console.log('✅ KPI data captured successfully from visible DOM');
        } else {
          console.warn('⚠️ KPI container not found in DOM');
        }
      } catch (err) {
        ...
        kpiSummaryHTML = `<div class="placeholder-content" style="background:#fff3cd; border:2px solid #ffc107; padding:20px; border-radius:8px; margin:20px;">
          ...
          <p style="margin:10px 0;"><strong>Note:</strong> KPI data could not be captured. Please ensure you've opened the Divisional KPIs view before exporting.</p>
        </div>`;
      }

      // 🎯 Capture table data from DOM (no navigation needed)
      console.log('🔥 Step: Capturing table data from current DOM...');
      
      let productGroupTableHTML = '<div class="placeholder-content"><h3>Product Group</h3><p>Not available - please open Product Group view before exporting.</p></div>';
      let plFinancialTableHTML = '<div class="placeholder-content"><h3>P&L Financial</h3><p>Not available - please open P&L view before exporting.</p></div>';
      let salesCustomerTableHTML = '<div class="placeholder-content"><h3>Sales by Customer</h3><p>Not available - please open Sales by Customer view before exporting.</p></div>';
      let salesRepTableHTML = '<div class="placeholder-content"><h3>Sales by Sales Rep</h3><p>Not available - please open Sales by Sales Rep view before exporting.</p></div>';
      let salesCountryTableHTML = '<div class="placeholder-content"><h3>Sales by Country</h3><p>Not available - please open Sales by Country view before exporting.</p></div>';
      
      // Try to find tables in the current DOM
      const productGroupTable = document.querySelector('table.product-group-table');
      if (productGroupTable) {
        const cloned = productGroupTable.cloneNode(true);
        productGroupTableHTML = cloned.outerHTML;
        console.log('✅ Product Group table captured');
      }
      
      const plTable = document.querySelector('table.pl-financial-table, table.financial-table');
      if (plTable) {
        const cloned = plTable.cloneNode(true);
        plFinancialTableHTML = cloned.outerHTML;
        console.log('✅ P&L Financial table captured');
      }
      
      const salesCustomerTable = document.querySelector('table.sales-by-customer-table');
      if (salesCustomerTable) {
        const cloned = salesCustomerTable.cloneNode(true);
        salesCustomerTableHTML = cloned.outerHTML;
        console.log('✅ Sales by Customer table captured');
      }
      
      const salesRepTable = document.querySelector('table.sales-by-sales-rep-table');
      if (salesRepTable) {
        const cloned = salesRepTable.cloneNode(true);
        salesRepTableHTML = cloned.outerHTML;
        console.log('✅ Sales by Sales Rep table captured');
      }
      
      const salesCountryTable = document.querySelector('table.sales-by-country-table');
      if (salesCountryTable) {
        const cloned = salesCountryTable.cloneNode(true);
        salesCountryTableHTML = cloned.outerHTML;
        console.log('✅ Sales by Country table captured');
      }
...
${echartsBundle}
    ...
    @media (orientation:landscape) and (max-width:767px){ .orientation-hint { display:none !important; } }
        ...
        /* Below GP Expenses responsive scrolling for portrait mode */
        @media (max-width: 768px) and (orientation: portrait) {
            #full-screen-below-gp-expenses .modern-margin-gauge-panel {
                overflow-x: auto;
                overflow-y: visible;
                -webkit-overflow-scrolling: touch;
                padding-bottom: 28px;
            ...
            #full-screen-below-gp-expenses .full-screen-chart-container {
                display: inline-block;
            }

            #full-below-gp-expenses-chart {
                overflow: visible;
            }
        }
      ...
      .revenue-drivers .kpi-value ol li { padding:6px 10px; margin-bottom:6px; }
      .revenue-drivers > div > div { padding:10px 12px; margin-bottom:12px; }
      .revenue-driver-medal { font-size:1.6em; margin-right:10px; }
             ...
             if (targetChart) {
                 ...
                 setTimeout(function() {
                     if (chartType === 'divisional-kpis') {
                         ...
                         if (typeof initializeCombinedTrends === 'function') {
                             initializeCombinedTrends();
                         }
                     } else {
                         // For ECharts-based charts, check if already initialized
                         if (typeof charts !== 'undefined' && charts[chartType] && !charts[chartType].isDisposed()) {
                             console.log('Chart already initialized, just showing:', chartType);
                             charts[chartType].resize();
                         } else if (typeof initializeFullScreenChart === 'function') {
                             console.log('Chart not pre-initialized, initializing now:', chartType);
    ...
    function getResponsiveFlags() {
        ...
        // to ensure they're available immediately when the page loads
         ...
         function initializeFullScreenChart(chartType) {
             ...
             // Special handling for margin-analysis (doesn't use ECharts)
             if (chartType === 'margin-analysis') {
                 console.log('🎯 Initializing Margin Analysis (SVG gauges)');
                 setTimeout(function() {
             ...
             var manufacturingTotalsResult = null;
             var manufacturingFallbackSummary = '';
             var belowTotalsResult = null;
             var belowFallbackSummary = '';

             if (chartType === 'manufacturing-cost') {
                 manufacturingTotalsResult = renderManufacturingCostTotals();
                 manufacturingFallbackSummary = createTotalsFallbackSummary('Manufacturing Cost Summary', manufacturingTotalsResult);
             } else if (chartType === 'below-gp-expenses') {
                 belowTotalsResult = renderBelowGPExpensesTotals();
                 belowFallbackSummary = createTotalsFallbackSummary('Below GP Expenses Summary', belowTotalsResult);
             }

             if (!initializeChartContainer('full-' + chartType + '-chart', chartTitles[chartType] || chartType)) {
                 if (chartType === 'manufacturing-cost' && manufacturingFallbackSummary) {
                     chartDom.innerHTML = manufacturingFallbackSummary;
                 } else if (chartType === 'below-gp-expenses' && belowFallbackSummary) {
                     chartDom.innerHTML = belowFallbackSummary;
                 }
        ...
        function getSalesVolumeOption() {
            ...
            var bottomSpace = isSmallMobile ? 100 : isMobile ? 90 : 60;
        ...
        function renderMarginAnalysisGauges() {
         ...
         function renderManufacturingCostTotals() {
            ...
            if (!totalsContainer) {
             ...
             // Responsive container for totals cards
            var width = window.innerWidth;
            var height = window.innerHeight;
            var isMobile = width <= 768;
            var isPortrait = height > width;
            var isTablet = width > 768 && width <= 1100;
            var isLandscape = width >= height;
            var isPortraitMobile = isMobile && isPortrait;
            var isTabletLandscape = isTablet && isLandscape;

            var gap = isPortraitMobile ? '12px' : '5px';
            var flexWrap = isPortraitMobile ? 'nowrap' : isTabletLandscape ? 'nowrap' : 'wrap';
            var alignItems = isPortraitMobile ? 'stretch' : isTabletLandscape ? 'center' : 'flex-start';
            var justifyContent = isPortraitMobile ? 'flex-start' : isTabletLandscape ? 'flex-start' : 'space-around';
            var connectorWidth = isPortraitMobile ? 56 : isTabletLandscape ? 48 : 40;

            var containerStyle = 'gap:' + gap + ';';
            if (isPortraitMobile) {
                containerStyle += 'flex-direction:column;flex-wrap:nowrap;align-items:stretch;justify-content:flex-start;overflow-x:hidden;';
            } else if (isTabletLandscape) {
                containerStyle += 'flex-direction:row;flex-wrap:nowrap;align-items:' + alignItems + ';justify-content:' + justifyContent + ';overflow-x:auto;overflow-y:hidden;white-space:nowrap;';
            } else {
                containerStyle += 'flex-wrap:' + flexWrap + ';align-items:' + alignItems + ';justify-content:' + justifyContent + ';';
            }

            var totalsHTML = '<div class="totals-scroll-container manufacturing-totals-scroll" style="' + containerStyle + '">';
             ...
             periodsToUse.forEach(function(period, index) {
                ...
                var cardStyle = 'background-color: ' + color + '; border: 1px solid ' + color + ';';
                if (isPortraitMobile) {
                    cardStyle += 'width:100%;max-width:100%;min-width:0;';
                } else if (isTabletLandscape) {
                    cardStyle += 'min-width:220px;max-width:220px;';
                }

                var cardHTML = '';
                cardHTML += '<div class="manufacturing-totals-card" style="' + cardStyle + '">';
                cardHTML += '<div style="font-size: 14px; color: ' + textColor + '; font-weight: 500; margin-top: 8px;">' + periodName + '</div>';
                cardHTML += '<div style="font-weight: bold; font-size: 22px; color: ' + textColor + '; margin-top: 8px;">';
                ...
                var connectorHTML = '';
                 if (index < periodsToUse.length - 1) {
                    ...
                    var connectorStyle;
                    if (isPortraitMobile) {
                        connectorStyle = 'min-width:100%;width:100%;height:auto;padding:6px 0;flex-direction:row;gap:6px;';
                    } else {
                        connectorStyle = 'min-width:' + connectorWidth + 'px;width:' + connectorWidth + 'px;';
                    }

                    connectorHTML += '<div class="totals-connector' + (isPortraitMobile ? ' portrait' : '') + '" style="' + connectorStyle + '">';
                    if (variance === null || isNaN(variance)) {
                        var neutralStyle;
                        if (isPortraitMobile) {
                            neutralStyle = 'color: #888; font-size: 16px; font-weight: bold; text-align: center;';
                        } else if (isTabletLandscape) {
                            neutralStyle = 'color: #888; font-size: 18px; font-weight: bold; text-align: center;';
                        } else {
                            neutralStyle = 'color: #888; font-size: 16px; font-weight: bold; text-align: center;';
                        }
                       connectorHTML += '<span style="' + neutralStyle + '">0%</span>';
                     } else {
                       var arrowSize = isPortraitMobile ? '20px' : isTabletLandscape ? '22px' : '22px';
                       var valueSize = isPortraitMobile ? '18px' : isTabletLandscape ? '20px' : '18px';
                       var percentSize = isPortraitMobile ? '16px' : isTabletLandscape ? '18px' : '16px';
                       connectorHTML += '<span style="font-size: ' + arrowSize + '; font-weight: bold; color: ' + badgeColor + '; line-height: 1;">' + arrow + '</span>';
                       connectorHTML += '<span style="font-size: ' + valueSize + '; font-weight: bold; color: ' + badgeColor + '; line-height: 1.1;">' + Math.abs(variance).toFixed(1) + '</span>';
                       connectorHTML += '<span style="font-size: ' + percentSize + '; font-weight: bold; color: ' + badgeColor + '; line-height: 1.1;">%</span>';
                    }
                    connectorHTML += '</div>';
                } else if (isPortraitMobile) {
                    var placeholderStyle = 'min-width:100%;width:100%;height:auto;padding:6px 0;flex-direction:row;gap:6px;opacity:0;pointer-events:none;';
                    connectorHTML += '<div class="totals-connector portrait" style="' + placeholderStyle + '"><span style="font-size:20px;font-weight:bold;">▲</span><span style="font-size:18px;font-weight:bold;">0.0</span><span style="font-size:16px;font-weight:bold;">%</span></div>';
                }

                totalsHTML += '<div class="totals-item-wrapper">' + cardHTML + connectorHTML + '</div>';
             });
        ...
        function renderBelowGPExpensesTotals() {
            ...
            var gap = isPortraitMobile ? '12px' : '5px';
            var flexWrap = isPortraitMobile ? 'nowrap' : isTabletLandscape ? 'nowrap' : 'wrap';
            var alignItems = isPortraitMobile ? 'stretch' : isTabletLandscape ? 'center' : 'flex-end';
            var justifyContent = isPortraitMobile ? 'flex-start' : isTabletLandscape ? 'flex-start' : 'space-around';
            var connectorWidth = isPortraitMobile ? 56 : isTabletLandscape ? 48 : 40;
            ...
            var totalsHTML = '<div class="totals-scroll-container below-gp-totals-scroll" style="' + containerStyle + '">';
            ...
            periodsToUse.forEach(function(period, idx) {
                ...
                var cardHTML = '';
                cardHTML += '<div class="below-gp-expenses-totals-card" style="' + cardStyle + '">';
                cardHTML += '<div style="font-size:14px;color:' + textColor + ';font-weight:500;margin-top:8px;">' + periodName + '</div>';
                cardHTML += '<div style="font-weight:bold;font-size:22px;color:' + textColor + ';margin-top:8px;">';
                cardHTML += getUAEDirhamSymbolHTML() + ' ' + formattedMillions + 'M';
                cardHTML += '</div>';
                ...
                if (idx < periodsToUse.length - 1) {
                    ...
                    if (isPortraitMobile) {
                        ...
                        connectorStyle = 'min-width:' + connectorWidth + 'px;width:' + connectorWidth + 'px;height:100%;display:flex;flex-direction:column;justify-content:center;';
                    } else {
                        connectorStyle = 'min-width:' + connectorWidth + 'px;width:' + connectorWidth + 'px;';
                    }
                    ...
                    var placeholderConnector = 'min-width:100%;width:100%;height:auto;padding:6px 0;flex-direction:row;gap:6px;opacity:0;pointer-events:none;';
                    connectorHTML += '<div class="totals-connector portrait" style="' + placeholderConnector + '"><span style="font-size:20px;font-weight:bold;">▲</span><span style="font-size:18px;font-weight:bold;">0.0</span><span style="font-size:16px;font-weight:bold;">%</span></div>';
                }
        ...
        function renderSalesCountry() {
            ...
            var capturedSalesCountryHTML = salesCountryTableHTML;
        ...
        function initializeCombinedTrends() {
            ...
            function buildRowContainerStart(desktopStyle, mobileAlign, mobileMinWidth) {
                if (useHorizontalScroll) {
                    ...
                    return '<div class="combined-trend-scroll" style="width:100%;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;margin:20px 0 0 0;padding:0 16px 16px 16px;box-sizing:border-box;touch-action:pan-y pan-x;">' +
                        '<div style="display:flex;flex-wrap:nowrap;align-items:' + align + ';gap:' + scrollGap + 'px;' + minWidthStyle + '">';
                }
                return '<div style=' + JSON.stringify(desktopStyle) + '>';
            }
            ...
            combinedHTML += buildRowContainerStart('display: flex; flex-wrap: nowrap; align-items: center; gap: 5px; margin-top: 20px; margin-bottom: 0; width: 100%; padding: 0 24px;', 'center', mobileRowWidth);
            ...
            expensesCards.forEach(function(card, idx) {
                ...
                if (idx < expensesCards.length - 1) {
                    ...
                    combinedHTML += '<div style="' + connectorFlexStyle + ' display: flex; flex-direction: column; align-items: center; justify-content: center;">';
                    if (variance === null || isNaN(variance)) {
                    ...
                    combinedHTML += '<div style="' + connectorFlexStyle + '"></div>';
                }
            });
            ...
            PROFIT_KPIS.forEach(function(kpi, rowIdx) {
                ...
                combinedHTML += buildRowContainerStart('display: flex; flex-wrap: nowrap; align-items: center; gap: 5px; margin-top: 10px; margin-bottom: 0; padding-bottom: 0; width: 100%; padding: 0 24px;', 'center', mobileRowWidth);
        ...
        window.addEventListener('DOMContentLoaded', function() {
            ...
            waitForECharts(function() {
                ...
                try {
                    renderMarginAnalysisGauges();
                    console.log('✅ Margin Analysis gauges rendered');
                } catch (error) {
                    console.error('❌ Error rendering Margin Analysis:', error);
                }

                try {
                    var manufacturingChartInit = initializeFullScreenChart('manufacturing-cost');
                    if (manufacturingChartInit) {
                        console.log('✅ Manufacturing Cost chart initialized with totals');
                    }
                } catch (error) {
                    console.error('❌ Error initializing Manufacturing Cost chart:', error);
                }

                try {
                    var belowChartInit = initializeFullScreenChart('below-gp-expenses');
                    if (belowChartInit) {
                        console.log('✅ Below GP Expenses chart initialized with totals');
                    }
                } catch (error) {
                    console.error('❌ Error initializing Below GP Expenses chart:', error);
                }
                ...
                // Pre-initialize the 4 main charts so they're ready when user clicks
                console.log('📊 Pre-initializing main charts...');

                try {
                    initializeFullScreenChart('sales-volume');
                    console.log('✅ Sales & Volume Analysis chart initialized');
                } catch (error) {
                    console.error('❌ Error initializing Sales & Volume Analysis:', error);
                }

                try {
                    initializeFullScreenChart('margin-analysis');
                    console.log('✅ Margin Analysis chart initialized');
                } catch (error) {
                    console.error('❌ Error initializing Margin Analysis:', error);
                }
