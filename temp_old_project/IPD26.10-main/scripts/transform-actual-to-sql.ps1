...
function Test-ExcelStructure {
    ...
    $requiredColumns = @(
        ...
        'salesrepname',
        'customername',
        'COUNTRYNAME',  # Uppercase in Excel
        'PGCombine',    # Maps to productgroup
        'Material',
        'Process',
        'values_type',
        'Total'         # Maps to values
    )
...
function Backup-ExistingData {
    ...
    $countBeforeQuery = "SELECT COUNT(*) FROM public.fp_data_excel WHERE division = '$($Division.ToUpper())' AND type = 'Actual';"
...
"@
    ...
    Write-Log "Backup created: $($countBefore.Trim()) records backed up (ALL existing FP Actual data)" -Level "SUCCESS"
}
...
function Remove-ExistingData {
    ...
    $beforeCount = Invoke-SqlCommand -Query "SELECT COUNT(*) FROM public.fp_data_excel WHERE division = '$($Division.ToUpper())' AND type = 'Actual';" -ReturnOutput
...
"@
    ...
    $afterCount = Invoke-SqlCommand -Query "SELECT COUNT(*) FROM public.fp_data_excel WHERE division = '$($Division.ToUpper())' AND type = 'Actual';" -ReturnOutput
